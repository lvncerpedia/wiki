...
git worktree add ../verification verification
```
...
```bash
git worktree add ../feature-aaa -b feature/aaa main
```
...
もし今この bare リポジトリ自体が `main` を指してる状態だと、
...
そのときは、bare 側の HEAD を別ブランチに逃がす：

```bash
git symbolic-ref HEAD refs/heads/verification
```

bare + worktree 構成、だいぶ分かってきた感じするね。
これ慣れると「ブランチ＝ディレクトリ」感覚で並べられてめっちゃ快適だよ。ちょっと職人っぽくて好き。
